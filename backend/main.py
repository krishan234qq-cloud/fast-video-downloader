...
def _export_cookies_bypass_lock(browser: str) -> str | None:
    ...
    try:
        ...
        is_lock = any(k in str(direct_err).lower() for k in ["locked", "database", "permission", "access denied", "readonly"])
        if not is_lock:
            try:
    ...
    try:
        ...
        kwargs = {"cookie_file": tmp_cookie_path}
        if tmp_key_path:
            kwargs["key_file"] = tmp_key_path
