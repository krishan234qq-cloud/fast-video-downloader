...
def _parse_and_write_custom_cookies(cookies_str: str) -> str | None:
    if not cookies_str or not cookies_str.strip():
        return None
    tmp_fd, tmp_path = tempfile.mkstemp(suffix="_custom_cookies.txt")
    os.close(tmp_fd)
    content = cookies_str.strip()
        ...
        try:
            ...
            with open(tmp_path, "w", encoding="utf-8") as f:
                ...
                for cookie in cookies_list:
                    ...
                    expires = str(int(cookie.get("expirationDate") or cookie.get("expiry") or 0))
...
def _extract_info_with_fallback(url: str, browser: str = None, user_agent: str = None, custom_cookies: str = None):
    ...
    cookies_file = _parse_and_write_custom_cookies(custom_cookies)
    if not cookies_file:
        cookies_file = _export_cookies_bypass_lock(browser)
...
async def get_video_info(request: InfoRequest):
    try:
        info, used_generic = _extract_info_with_fallback(
            request.url, request.browser, request.user_agent, request.custom_cookies
        )
...
async def download_video(request: DownloadRequest):
    ...
    cookies_file = _parse_and_write_custom_cookies(request.custom_cookies)
    if not cookies_file:
    ...
    if sk_match:
        ...
        try:
            probe_opts = _build_ydl_opts(request.browser, request.user_agent, force_generic=False, cookies_file=cookies_file)
